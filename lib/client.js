window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      /* Native-app bridge: the DSH Mobile app (UA marker "DSHMobileApp") renders
         its own whale button inside a draggable native dock and calls this hook
         to toggle the sidebar. In app mode the in-page whale button is skipped
         so there is exactly one whale. Browser mode is unchanged. */
      var isNativeApp = navigator.userAgent.indexOf("DSHMobileApp") !== -1;
      window.__dshwmToggleSidebar = toggleSidebar;
      ...
      if (!isNativeApp) document.body.append(whale);
      ...
      ctx.effect(function () {
        return function () {
          tag.remove();
          whale.remove();
          if (window.__dshwmToggleSidebar === toggleSidebar) delete window.__dshwmToggleSidebar;
